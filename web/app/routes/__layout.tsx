...
export default function Layout() {
  ...
  return (
    <>
      <header className="w-full h-18 border-b  pt-1 relative">
        <div className="text-center text-xl mb-1">
          ...
          {result.success ? result.user.nickname : "Who are you?"}
        </div>
        <nav className="flex gap-4 px-4">
          ...
          {result.success ? (
            <>
              <Link className="text-blue-700 hover:underline" to="/logout">
                logout
              </Link>
            </>
          ) : (
